            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {courses.length > 0 ? (
                    courses.map(course => {
                        ...
                        const imageUrl = `http://localhost:4200/uploads/${course.image}`;
                        ...
                        return (
                            <li key={course._id} className="bg-white p-5 rounded-lg shadow-md hover:shadow-xl transition-shadow">
                                ...
                                {course.image ? (
                                    <img
                                        src={imageUrl} // Use the logged URL
                                        alt={course.name}
                                        className="w-full h-auto rounded-md mb-4"
                                        onError={(e) => {
                                            e.target.onerror = null; 
                                            e.target.src = '/path/to/default-image.jpg'; // Ensure this is a valid fallback image path
                                        }} 
                                    />
                                ) : (
                                    <img
                                        src="/path/to/default-image.jpg" // Ensure this is a valid path
                                        alt="Default Image"
                                        className="w-full h-auto rounded-md mb-4"
                                    />
                                )}
